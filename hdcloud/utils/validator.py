...
"""
参考自 url: https://github.com/mansam/validator.py
"""
__doc__ = "入参校验装饰器"
__version__ = "1.2.8"
...
class InCartesian(Validator):
    ...
    def __call__(self, value):
        lst = [str(i) for i in self.collection]
        arr = str(value).split(self.split)
        for v in arr:
            if not v in lst:
                return False
        return True
...
class IsSplitDigit(Validator):
    ...
    # Example:
        validations = {
            "field": [IsSplitDigit(True)]
        }
        passes = {"field": "123,234"}
        fails  = {"field": "30,etd"}

    """

    def __init__(self, enable_null=False, split=","):

        self.enable_null=enable_null
        self.split=split
        self.err_message = "must match numbers and [%s]" % self.split
        self.not_message = "must not match numbers and [%s]" % self.split

    def __call__(self, value):
        if self.enable_null and (value == None or value==""):
            return True
        if self.split in value:
            pattern = "(\\d*)(%s\\d*)*(\d$)" % self.split
        else:
            pattern= "^\d$"
    ...
    def decorator(f):
        ...
        def decorated_func(*args, **kwargs):
            ...
            try:
                ...
                if not result:
                    return jsonify({"code": 500, "data": None, "msg": err})
            except Exception as e:
...
def limits(dict_args, strip, modify, default, diy_func, rules):
    ...
    if dict_args.get("form", False) or dict_args.get("values", False):
        ...
        if not result:
            return result, err
    if not result and (dict_args.get("args", True) or dict_args.get("values", False)):
